...
### 추가
- **`kupid_syllabus` — 학사 시스템 강의계획서 조회. 로그인이 필요 없다.**
  평가 비중(중간·기말·과제 %), 주차별 강의계획, 교재·참고문헌, 성적평가 방식
  (절대/상대), 강의시간·강의실, 담당교수 연락처·면담시간을 반환한다.

  v0.13.0에서 `kupid_get_syllabus`를 "학사 시스템에서 메뉴가 사라졌다"며 제거하고
  `kupid_lms_syllabus`(Canvas)로 대체했었다. 실제로는 사라진 게 아니라 AMS로
  옮겨가 **인증 없이** 열려 있었다. LMS 쪽은 수강 중인 과목만, 그것도 개강
  이후에나 보이므로 수강신청 시기에는 쓸 수 없다는 한계가 있었다.

### 알아둘 점
- 이 API는 **틀린 조건에 오류 대신 빈 목록으로 답한다.** 두 가지를 조심해야 한다.
  - 평가항목·주차별 계획은 기본정보(`findAllLctreSyllaRegPopList.do`)가 아니라
    `findAllLctreSyllaPopLrnPlanList.do`에 있다. 기본정보의 배점 필드
    (`tb1000`~`tbTot`)는 **계획서가 채워진 과목에서도 늘 0**이라, 이것만 보면
    "교수가 아직 안 썼다"고 오판하게 된다.
  - 세부 조회는 개설학과 코드(`estblDeprtCd`)가 정확해야 내용을 채워 준다
    (같은 대학원 안에서도 학과마다 다르다 — AAI 7313, BDC 7309).
    `fetch_syllabus()`는 기본정보 응답에 담겨 오는 실제 학과 코드를 받아
    두 번째 호출에 쓰는 방식으로 이 함정을 없앴다.
- 계절학기에는 강의계획서가 없다시피 해서, 학기를 생략하면 방학 중일 때
  다가오는 정규학기로 넘긴다(여름 → 그 해 2학기, 겨울 → 다음 학년도 1학기).
- 과목이 미개설이면 실패로, 개설됐지만 교수가 아직 작성하지 않았으면 성공에
  `note`를 담아 구분해 돌려준다.

## [0.18.1] - 2026-08-10

### 문서
- **인증 코드 수신 이메일에 대한 안내 추가.** 어디로 오는지, 어떻게 바꾸는지,
  안 오면 어떻게 하는지가 어디에도 적혀 있지 않았다. README에 표로 정리하고
  `kupid_ams_auth_start` 설명에도 넣어, 도구가 사용자에게 바로 안내할 수 있게 했다.

  - 코드는 **KUPID 포털에 등록된 이메일**로 발송된다. 따로 등록할 것은 없다
  - 주소 변경은 `포털 > My Page > 개인정보 수정`
  - 메일이 안 보이면 스팸함 확인 (학교 안내에도 명시된 흔한 경우)
  - 이 서버는 메일을 보내지 않는다. 발송 요청만 하고(요청 바디에 주소를 싣지
    않는다) 학교가 세션으로 사용자를 식별해 등록된 주소로 직접 보낸다.
    응답에는 마스킹된 주소만 담긴다

  기존 안내가 「휴대폰 인증 신규 등록」을 가리키고 있어 실제 구현(이메일 OTP)과
  어긋나 있던 것도 바로잡았다.
